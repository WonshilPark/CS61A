...
def balanced(m):
    ...
    if is_weight(m):
    	...
    	left_t = total_weight(end(left(m))) * length(left(m))
    	right_t = total_weight(end(right(m))) * length(right(m))
    	return balanced(end(left(m))) and balanced(end(right(m))) and left_t == right_t 
...
def make_fib():
    ...
    next_one = 1
    def fib():
    	nonlocal previous, next_one
    	fib = previous
    	previous, next_one = next_one, previous + next_one
    	return fib
    return fib
