...
(define (derive-sum expr var)
  (if (sum? expr) 
  	(begin (define addend1 (addend expr)) (define augend1 (augend expr))
  		(make-sum (derive addend1 var) (derive augend1 var))))
)

(define (derive-product expr var)
  (if (product? expr)
  	(begin (define multi (multiplier expr)) (define multi_d (multiplicand expr))
  		(make-sum (make-product (derive multi var) multi_d) (make-product (derive multi_d var) multi))))
)
...
(define (derive-exp exp var)
	(make-product (make-product (exponent exp) (make-exp (base exp) (make-sum (exponent exp) -1)))
		(derive (base exp) var))
)