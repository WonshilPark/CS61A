...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        one_argument = rest.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(operator, one_argument, env)
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return '#t'
    elif len(expressions) == 1 or scheme_falsep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.first, env, True)
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return '#f'
    elif len(expressions) == 1 or scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.first, env, True)
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formals, vals = Pair(bindings.first.first, formals), Pair(scheme_eval(bindings.first.rest.first, env), vals)
